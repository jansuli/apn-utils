...
def notInCosetProduct(reprTuple, subspace, vectorIndices, orderedDomains):
	cosets = []
	print("Avoid cosets %s."%str(reprTuple))
	for i in range(len(reprTuple)):
		rep = reprTuple[i]
		domain = orderedDomains[i]
		coset = Set( [rep + w for w in subspace] )
		
		relevant = domain.intersection(coset)
		
		if not relevant.is_empty():
			cosets.append( relevant )
		else:
			print("BREAK")
			break
	else:
		...
		for fieldTuple in fieldTuples:
			...
			clauses += 1
	#expression = expression[:-1]
		print("Added %d clauses."%clauses)
	return expression, clauses
	
def sumNotInSet(outSet, ind, orderedDomains):
	vectorIndices = []
	for i in ind:
	...
	for var in variations:
			if sum (var) == 0:
				...
				subexpression, subclauses = notInCosetProduct(var, outSet, vectorIndices, orderedDomains)
...
def generateSatString(submatrix):
	...
	for i in range(0, N):
		...
		print len(rowSpace)
		domains.append(KSet.difference(rowSpace))
	...
	if True:	
		...
		for ind in sumIndices:
			...
			subexpression, subclauses = sumNotInSet(rowSpace, ind, [domains[i] for i in ind])
			expression += subexpression
			clauses += subclauses
